...
def select_best_enrichment_candidate(input_sentence, candidates, model_encoder_only):
    enriched = False
    try:
        ...
        enriched = True
        enriched_sentence = candidates[top_results[1]]   
    except:
...
def main():
    ...
    for input_sentence, candidates in tqdm(list_input_and_sentences_candidates):
        if len(candidates) == 0:
            ...
            enriched, sentence_enriched = select_best_enrichment_candidate(input_sentence, candidates, model_encoder_only)
    ...
    output_sentfin_enriched.to_csv(f'{path}/OUPTUT/sentfin_enriched_ablation_no_perplexity.csv')
