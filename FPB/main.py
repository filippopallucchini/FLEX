...
def make_up_score(cos_sim_score, perplexity_score, max_perplexity_score, weight_coefficient):
    return weight_coefficient*cos_sim_score + (1-weight_coefficient)*(1-(math.log((1+perplexity_score), 10)/(math.log((1+max_perplexity_score), 10))))
...
def select_best_enrichment_candidate(input_sentence, candidates, model_encoder_only, limit_semantic_enr, weight_coefficient):
    enriched = False
    try:
        ...
        input_sentence_perplexity = results_perplexities['perplexities'][-1]
        dict_make_up_scores = {}
        for i in range(len(candidates)):
            cos_sim_score = cos_scores[i]
            perplexity_score = results_perplexities['perplexities'][i]
            if cos_sim_score < limit_semantic_enr or perplexity_score > input_sentence_perplexity:
                continue
            else:
                max_perplexity_score = max(results_perplexities['perplexities'])
                dict_make_up_scores[candidates[i]]= make_up_score(cos_sim_score, perplexity_score, max_perplexity_score, weight_coefficient)
        
        if len(dict_make_up_scores) == 0:
            enriched = False
            enriched_sentence = input_sentence
        else:
            dict_make_up_scores_ordered = {k: v for k, v in sorted(dict_make_up_scores.items(), key=lambda x: x[1], reverse=True)}
            enriched = True
            enriched_sentence = list(dict_make_up_scores_ordered.keys())[0]
    except:
        enriched_sentence = input_sentence
...
def main():
    ...
    #ENCODER ONLY
    model_name_encoder_only = 'sentence-transformers/all-mpnet-base-v2'
    ...
    if check_presence_candidates == False:
        ...
        list_input_and_sentences_candidates = pickle.load(open(f"{path}/DATA/file_{num_candidates}_candidates_for_enrichment.pkl", 'rb'))
    ...
    for input_sentence, candidates in tqdm(list_input_and_sentences_candidates):
        if len(candidates) == 0:
            ...
            enriched, sentence_enriched = select_best_enrichment_candidate(input_sentence, candidates, model_encoder_only, limit_semantic_enr, weight_coefficient)
    ...
    output_fpb_enriched.to_csv(f'{path}/OUTPUT/fpb_enriched.csv')
